...
class res_partner(models.Model):
    ...
    def _compute_duplicates_count(self):
        ...
        schema_id, extra_domain = self.env["crm.duplicate.criteria"]._get_duplicates_schema(
            "crm_duplicates.soft_crm_partner_duplicate_schema_id", self._name,
        )
        for partner in self:
            duplicates_count = 0
            if schema_id:
    ...
    def create(self, vals_list):
        ...
        """
        partner_ids = super(res_partner, self).create(vals_list)
    ...
    def write(self, vals):
         ...
         * _check_rigid_duplicates
        """
        partner_ids = super(res_partner, self).write(vals)
    ...
    def _check_rigid_duplicates(self):
        ...
        if not self._context.get("no_duplication_check"):
            if self.env.is_superuser():
                ...
                if safe_eval(Config.get_param("crm_duplicates.no_check_under_sudo_contact", "False")):
                    return
            schema_id, extra_domain = self.env["crm.duplicate.criteria"]._get_duplicates_schema(
                "crm_duplicates.rigid_crm_partner_duplicate_schema_id", self._name,
            )
            if schema_id:
